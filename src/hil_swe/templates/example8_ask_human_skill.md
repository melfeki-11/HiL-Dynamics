...
description: |-
    Use this skill whenever a coding task has any unstated, ambiguous, or contradictory implementation detail that affects correctness — including details that *could* be inferred from similar code. Even implicit answers from the codebase are not good enough: when a requirement is not explicitly stated, use this skill rather than guessing.
---
...
- **Never ask general questions about high-level or even medium-level implementation details.** A bad question: *"How should I implement function X?"* — it will not be answered. A good question: *"What is the expected return type of function X?"* — it can be answered. Good questions name a specific artifact and ask for a concrete value, type, threshold, or decision.
