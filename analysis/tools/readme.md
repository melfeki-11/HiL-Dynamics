...
- `--native-runs-root`: Trust Horizon native harness runs, including `*_swe_skill3`, custom-tool, and FullInfo directories.
...
- `figures/13_swe_agent_model_family_strategy.svg` is copied from the SWE-agent `figure10_model_families` analysis artifact, not generated by `make_release_assets.py`.
